...
function Tickets() {
  ...
  const handleCreatePost = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/posts/create`, formData, {
        headers: {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <Button variant='contained' color='primary' onClick={handleOpenn}>
        Create Post
      </Button>
          ...
          {data.map(post => (
            <Grid item xs={12} key={post.id}>
              <StyledCard>
                ...
                <CardActions sx={{ justifyContent: 'space-between', backgroundColor:'#f1f1f1', width:'60%', display: 'flex', flexDirection: 'column'  }}>
                  {post.comments && post.comments.length > 0 && (
                    <Box sx={{ width: '100%', paddingTop: 2, overflow: 'scroll', overflowX: 'hidden', height: '400px' }}>
      ...
      >
        <Box sx={{ p: 3, maxWidth: 400, margin: 'auto', mt: '20%', bgcolor: 'background.paper', borderRadius: 2 }}>
          ...
          />
          <Button onClick={handleCreateComment} variant='contained' color='primary' sx={{ mt: 2 }}>
            Submit
          </Button>
        </Box>
      </Modal>
