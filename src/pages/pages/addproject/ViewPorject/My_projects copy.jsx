...
const MyProjects = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setProjects(data);
      setLoadingProjects(false); // Set loading to false when projects are fetched
    } catch (error) {
  ...
  const totalPages = Math.ceil(projects.filter(project => project.status === "Active").length / itemsPerPage);
  ...
  return (
    ...
    <div>
          {loadingProjects && (
      // Render a loading indicator or message while projects are being fetched
      <div></div>
    )} 
